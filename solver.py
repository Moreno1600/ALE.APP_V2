...
st.markdown("""
    ...
    /* Reduced gap between stacked buttons */
    div.stButton {
        margin-bottom: 8px; 
    }
    ...
    /* 1. Pull the Chat Input closer to the buttons */
    div[data-testid="stChatInput"] {
        margin-top: 10px !important;  /* Reduced from 30px */
        padding-top: 0px !important;  /* Reduced from 20px */
    }

    /* 2. Pull "Suggested Actions" closer to "AI Assistant" text */
    .section-label {
        font-size: 0.85rem;
        color: #94a3b8;
        margin-bottom: 8px;        /* Reduced from 15px (Closer to buttons) */
        margin-top: 5px;           /* Reduced from 20px (Closer to text above) */
        font-weight: 800;
...
def handle_ai_query(user_query, fin_data):
    ...
    try:
        ...
        Explain the 'Why' behind the numbers. Do NOT use LaTeX formatting (no dollar signs with backslashes).
        """
